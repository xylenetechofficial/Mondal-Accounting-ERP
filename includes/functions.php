...
12. getAllDevices()
13. getAllPartyData($table)
14. getSingleParty($pid)
...
class functions
{
    ...
    public function getSingleParty($pid)
    {

        $sql = "SELECT party_id FROM party WHERE party_id = '" . $pid . "'";
        $this->db->sql($sql);
        $res = $this->db->getResult();
        $party = array();
        foreach ($res as $row) {
            array_push($party, $row['party_id']);
        }
        return $party;
    }
}
