            ...
            <?php  
                ...
                for($cb=1;$cb<=$boxDisplay;$cb++){ 
                    if(($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)){ 
                        ...
                        global $db; 
                        $result = $db->query("SELECT month FROM grivance_records WHERE date = '".$currentDate."'"); 
...
function getEvents($date = ''){ 
    ...
    global $db; 
    $result = $db->query("SELECT month FROM grivance_records WHERE date = '".$date."'"); 
    if($result->num_rows > 0){ 
        ...
        while($row = $result->fetch_assoc()){ $i++; 
            $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">'.$i.'.</span>'.$row['month'].'</li>'; 
        } 
        $eventListHTML .= '</ul>'; 
    } 
    echo $eventListHTML; 
