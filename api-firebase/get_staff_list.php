...
if (isset($_POST['accesskey'])) {
    ...
    if ($access_key_received == $access_key) {
        $sql_query = "SELECT * FROM emp_joining_form WHERE emp_type_id = '1' ORDER BY name ASC";
        $db->sql($sql_query);
        $res = $db->getResult();
        if (!empty($res)) {
            ...
            foreach ($res as $r) {
                ...
                $db->sql("SELECT * FROM family WHERE emp_id = '" . $r['id'] . "' ORDER BY id DESC LIMIT $offset,$limit");
                $childs = $db->getResult();
                if (!empty($childs)) {
