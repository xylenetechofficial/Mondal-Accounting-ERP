...
if (isset($_POST['accesskey'])) {
    ...
    if ($access_key_received == $access_key) {
        $sql_query = "SELECT DISTINCT date FROM news ORDER BY id DESC";
        $db->sql($sql_query);
        $res = $db->getResult();
        $tmp = [];
        ...
        if (!empty($res)) {
            for ($i = 0; $i < count($res); $i++) {
                $res[$i]['date'] = $res[$i]['date'];
                //$res[$i]['web_image'] = (!empty($res[$i]['web_image'])) ? $res[$i]['web_image'] : '';
            }
            ...
            foreach ($res as $r) {
                ...
                $db->sql("SELECT * FROM news WHERE date = '" . $r['date'] . "' ORDER BY id DESC");
                $childs = $db->getResult();
                if (!empty($childs)) {
                    for ($i = 0; $i < count($childs); $i++) {
                        $r['childs'][$i] = array('title' => $childs[$i]['newsliner'], 'time' => $childs[$i]['time']);
                    }
                }
                $tmp[] = $r;
            }
            ...
            $response['message'] = "News Retrived Successfully!";
            $response['data'] = $res;
        } else {
            $response['error'] = true;
