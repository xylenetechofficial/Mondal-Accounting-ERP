...
if ((isset($_POST['type'])) && ($_POST['type'] == 'attendance_out_time')) {
    ...
    if ($shift_id == '1') {
        ...
        //$hours = round((strtotime($out_time) - strtotime($in_time))/3600, 1);
        //$hours = (strtotime($out_time) - strtotime($in_time))/3600;
        $shift_in_time = $today . ' 14:00:00';
        $shift_out_time = $today . ' 22:00:00';
        $shift_ext_in_time = $today . ' 14:10:00';
    ...
    $sql = "SELECT * FROM `emp_attendance_staff` WHERE emp_id = '$emp_id' AND date = '$date' ORDER BY id DESC limit 1";
    $db->sql($sql);
    $res = $db->getResult();
