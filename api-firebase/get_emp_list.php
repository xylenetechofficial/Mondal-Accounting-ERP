...
*/
$limit = (isset($_POST['limit']) && !empty($_POST['limit']) && is_numeric($_POST['limit'])) ? $db->escapeString($fn->xss_clean($_POST['limit'])) : 100;
...
if (isset($_POST['accesskey'])) {
    ...
    if ($access_key_received == $access_key) {
        $sql_query = "SELECT * FROM emp_joining_form ORDER BY name ASC LIMIT $limit OFFSET $offset";
        $db->sql($sql_query);
        $res = $db->getResult();
        if (!empty($res)) {
            ...
            foreach ($res as $r) {
                ...
                if (!empty($childs)) {
                    for ($i = 0; $i < count($childs); $i++) {
                        ...
                        $r['childs'][$i] = array('id' => $childs[$i]['id'], 'emp_id' => $childs[$i]['emp_id'], 'family_name' => $childs[$i]['family_name'], 'family_age' => $childs[$i]['family_age'], 'family_relation' => $childs[$i]['family_relation'], 'family_remark' => $childs[$i]['family_remark'], 'created_at' => $childs[$i]['created_at']);
                    }
                }
                $tmp[] = $r;
            }
            ...
            $response['message'] = "Employees List Retrived Successfully!";
            $response['data'] = $res;
        } else {
            $response['error'] = true;
