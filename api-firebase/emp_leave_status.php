...
if (isset($_POST['accesskey'])) {
    ...
    if ($access_key_received == $access_key) {
        $sql_query = "SELECT * FROM `labour_leave` WHERE emp_id = '$emp_id' ORDER BY 'id' DESC";
        $db->sql($sql_query);
        $res = $db->getResult();
        for ($i = 0; $i < count($res); $i++) {
            ...
            $date = $res[$i]['date'];
            $approved_by_emp_name = $res[$i]['approved_by_emp_name'];
            $approved_by_remark = $res[$i]['approved_by_remark'];
            //$tmp = [];
        }
        if (!empty($res)) {
