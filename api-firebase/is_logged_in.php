...
if (isset($_POST['accesskey'])) {
    ...
    if ($access_key_received == $access_key) {
        $sql_query = "SELECT emp_joining_form.*, emp_attendance.is_logged_in FROM `emp_joining_form` INNER JOIN `emp_attendance` ON emp_joining_form.id = emp_attendance.emp_id WHERE emp_attendance.date = '$date' ORDER BY name ASC";
        $db->sql($sql_query);
        $res = $db->getResult();
        if (!empty($res)) {
            ...
            foreach ($res as $r) {
                ...
                if (!empty($childs)) {
                    for ($i = 0; $i < count($childs); $i++) {
                        ...
                        //$r['childs'][$childs[$i]['family_name']] = (array)$childs[$i];
