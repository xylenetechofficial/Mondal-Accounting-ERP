...
*/
        if (!empty($name)) {
            ...
            $sql = "INSERT INTO products (size_chart,name,tax_id,slug,category_id,subcategory_id,location_id,location,image,other_images,description,shipping_delivery,indicator,manufacturer,made_in,return_status,cancelable_status, till_status,is_cod_allowed,total_allowed_quantity) VALUES('$upload_image1','$name','$tax_id','$slug','$category_id','$subcategory_id','$location_id','$location','$upload_image','$other_images','$description','$shipping_delivery','$indicator','$manufacturer','$made_in','$return_status','$cancelable_status','$till_status','$cod_allowed','$total_allowed_quantity')";
            $db->sql($sql);
...
*/
                        ...
                        $type = $_POST['type'];
                        $measurement = $_POST['packate_measurement'][$i];
                        $measurement_unit_id = $_POST['packate_measurement_unit_id'][$i];

                        $price = $_POST['packate_price'][$i];
                        $discounted_price = !empty($_POST['packate_discounted_price'][$i]) ? $db->escapeString($fn->xss_clean($_POST['packate_discounted_price'][$i])) : 0;
                        $serve_for = $_POST['packate_serve_for'][$i];
                        $stock = $_POST['packate_stock'][$i];
                        $serve_for = ($stock == 0 || $stock <= 0) ? 'Sold Out' : $serve_for;
                        $stock_unit_id = $_POST['packate_stock_unit_id'][$i];
                        $product_sku = $_POST['packate_product_sku'][$i];
                        $hsn_code = $_POST['packate_hsn_code'][$i];

                        $sql = "INSERT INTO product_variant (product_id,type,measurement,measurement_unit_id,price,discounted_price,serve_for,stock,stock_unit_id,images,sku,hsn_code) VALUES('$product_id','$type','$measurement','$measurement_unit_id','$price','$discounted_price','$serve_for','$stock','$stock_unit_id','$variant_other_images','$product_sku','$hsn_code')";
                        $db->sql($sql);
                    ...
                    for ($i = 0; $i < count($_POST['loose_measurement_unit_id']); $i++) {
                        ...
                        $product_id = $db->escapeString($res_inner[0]['id']);
                        $type = $_POST['type'];
                        $measurement = $_POST['loose_measurement'][$i];
                        $measurement_unit_id = $_POST['loose_measurement_unit_id'][$i];
                        $price = $_POST['loose_price'][$i];
                        $discounted_price = !empty($_POST['loose_discounted_price'][$i]) ? $db->escapeString($fn->xss_clean($_POST['loose_discounted_price'][$i])) : 0;
                        $serve_for = $_POST['serve_for'];
                        $stock = $_POST['loose_stock_name'];
                        $serve_for = ($stock == 0 || $stock <= 0) ? 'Sold Out' : $serve_for;
                        $stock_unit_id = $_POST['loose_stock_unit_id'];
                        $product_sku = $_POST['loose_product_sku'][$i];
            ...
            <div class="box box-primary">
                ...
                <form id='add_product_form' method="post" enctype="multipart/form-data">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            <label class="radio-inline"><input type="radio" name="type" id="packate" value="packet" checked></label>
